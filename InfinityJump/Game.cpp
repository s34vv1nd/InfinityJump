...
int Game::Init()
{
	...
	if (m_currentState == STATE_PLAY)
	{
		...
		int exitcode = Singleton<SceneManager>::GetInstance()->Init(SM_PLAY);
...
void Game::Draw()
{
	...
	else Singleton<SceneManager>::GetInstance()->Draw();
}
...
void Game::Update(GLfloat dt)
{
	if (m_currentState == STATE_PLAY)
	{
		...
		while (!m_pads.empty()) {
			...
			if (pad->getPos2D().x < -pad->getWidth() - 100) {
				SAFE_DEL(pad);
				m_pads.pop_front();
			}
			else break;
	...
	else Singleton<SceneManager>::GetInstance()->Update(dt);
}
