...
Vector4 Vector4::operator * (const GLfloat k)
{
...
GLfloat Vector4::operator [] (const unsigned int idx)
{
	return (&x)[idx];
}
...
Matrix::Matrix(const GLfloat val)
{
...
Matrix Matrix::operator * (const GLfloat k)
{
	Matrix mat( *this );
	mat *= k;
	return mat;
